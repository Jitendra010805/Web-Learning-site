...
export const UserContextProvider = ({ children }) => {
  ...
  async function loginUser(email, password, navigate, fetchMyCourse) {
    ...
    try {
      ...
      // Save to recent accounts for quick login
      const savedAccounts = JSON.parse(localStorage.getItem("recentAccounts")) || [];
      const newAccount = { email: data.user.email, name: data.user.name };
      const updatedAccounts = [newAccount, ...savedAccounts.filter(acc => acc.email !== data.user.email)].slice(0, 5);
